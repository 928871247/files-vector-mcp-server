...
def register_tools(mcp: FastMCP):
    ...
    def search(
        ...
        Args:
            query: 搜索查询文本
            top_k: 返回结果数量，默认5
            search_type: 搜索类型，"vector"或"fulltext"，默认"vector"
            topic: 可选，按主题过滤搜索结果
        ...
        try:
            ...
            if search_type == "vector":
                ...
                results = db_service.search_similar(query_vector, top_k, topic=topic)
            elif search_type == "fulltext":
                results = db_service.search_fulltext(query, top_k, topic=topic)
            else:
                return {"status": "error", "message": "search_type必须是'vector'或'fulltext'"}
...
def start_file_watcher():
    ...
    def start_worker_event_loop():
        ...
        asyncio.set_event_loop(worker_event_loop)
    ...
    # 启动事件循环线程
    loop_thread = Thread(target=start_worker_event_loop, name="EventLoopThread", daemon=True)
    loop_thread.start()
    worker_threads.append(loop_thread)
    logger.info("工作线程事件循环已启动")
    ...
    # 启动初始扫描线程
    initial_scan_thread = Thread(target=initial_scan, name="InitialScanThread")
    ...
    def batch_worker():
        ...
        def get_worker_loop():
            ...
            for thread in threading.enumerate():
                if thread.name == "EventLoopThread":
                    return worker_event_loop
            return None
        ...
        while True:
            try:
                ...
                if loop and not loop.is_closed():
                    ...
                    logger.error("无法发送事件：工作线程事件循环未运行")
    
    # 启动批处理工作线程
    batch_worker_thread = Thread(target=batch_worker, name="BatchWorkerThread")
